# Step 5c: Per-password crack time estimate at bcrypt(14) speed
# Ranks all 150 passwords by estimated crack time and plots a spectrum chart.
# Run: python3 scripts/step5c_crack_times.py
...
BCRYPT14_HPS = 1.0 / (188.877 / 150)
ROCKYOU_SIZE = 14_344_391

# Approximate RockYou rank for passwords cracked by dictionary or rules
ROCKYOU_RANKS = {
    "123456": 1, "password": 2, "123456789": 3, "12345678": 4, "12345": 5,
    "qwerty": 6, "1234567": 7, "111111": 8, "abc123": 9, "iloveyou": 10,
    "password1": 12, "admin": 15, "monkey": 18, "letmein": 20, "princess": 22,
    "dragon": 25, "sunshine": 30, "master": 35, "welcome": 40, "michael": 45,
    "jessica": 50, "login": 55, "football": 60, "shadow": 65, "superman": 70,
    "batman": 75, "soccer": 80, "trustno1": 90, "baseball": 95, "pass123": 100,
    "P@ssw0rd": 500, "P@ssword1": 600, "Passw0rd!": 800,
    "Adm1n@123": 1200, "B@tm@n99": 1500, "Dr@g0n99": 1800, "J3ssica!": 2000,
    "M0nk3y!1": 2200, "Pr1nc3ss!": 2500, "Sh@dow123": 2800,
    "Sup3rman1": 3000, "Tr0uble!2": 3500,
}

# All passwords cracked in our experiment (from pot files)
CRACKED = {
...
CAT_COLORS = {
    "common": "#e74c3c", "human_modified": "#e67e22", "short_complex": "#f1c40f",
    "passphrases": "#2ecc71", "random": "#27ae60",
}
CAT_LABEL = {
    ...
    with open(f"passwords/{cat}.txt") as f:
        for line in f:
            ...
            if pwd in ROCKYOU_RANKS:
                attack_sec = ROCKYOU_RANKS[pwd] / BCRYPT14_HPS
                method = "Dictionary/Rules"
            else:
                n = charset_size(pwd)
                attack_sec = ((n ** len(pwd)) / 2) / BCRYPT14_HPS
                method = "Brute-force"
            ...
            rows.append({
                ...
                "Category":              cat,
                "Length":                len(pwd),
                "Charset Size":          charset_size(pwd),
                "Attack Method":         method,
                "Est. Time (bcrypt14)":  fmt_time(attack_sec),
                "Cracked in Experiment": "YES" if pwd in CRACKED else "no",
...
with open("graphs/table_crack_time_per_password.csv", "w", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=fieldnames)
    writer.writeheader()
    for r in rows:
        writer.writerow({k: v for k, v in r.items() if not k.startswith("_")})
print("Saved -> graphs/table_crack_time_per_password.csv")
...
# Chart 10: Crack time spectrum scatter plot
plt.rcParams.update({
...
for r in rows:
    cat   = r["Category"]
    log_s = math.log10(max(r["_sort_sec"], 0.001))
    ...
    ax.scatter(log_s, y, color=CAT_COLORS[cat], marker=marker, s=size,
...
ax.set_yticklabels([CAT_LABEL[c] for c in CATEGORIES], fontsize=11)
ax.set_xlabel("Estimated Crack Time at bcrypt(14) Speed (log scale)", fontsize=11)
ax.set_title("Per-Password Crack Time Spectrum — bcrypt(14)\nX = Cracked   O = Survived",
             fontsize=13, fontweight="bold")

patches = [mpatches.Patch(color=CAT_COLORS[c], label=CAT_LABEL[c]) for c in CATEGORIES]
cracked_m  = plt.Line2D([0],[0], marker="X", color="w", markerfacecolor="black", markersize=10, label="Cracked")
survived_m = plt.Line2D([0],[0], marker="o", color="w", markerfacecolor="black", markersize=8,  label="Survived")
ax.legend(handles=patches + [cracked_m, survived_m], loc="lower right", fontsize=9)
